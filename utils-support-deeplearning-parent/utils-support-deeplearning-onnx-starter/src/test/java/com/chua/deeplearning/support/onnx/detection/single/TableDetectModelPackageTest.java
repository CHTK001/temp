...
/**
 * utils-support-models-onnx-table-detect 模型包资源测试。
 */
@DisplayName("utils-support-models-onnx-table-detect 资源验证")
class TableDetectModelPackageTest {

    private static final String CLASS_NAMES_RESOURCE = "vision/table/yolov8n/class.names.txt";
    private static final String PLACEHOLDER_FILE = "vision/table/yolov8n/.gitkeep";
    ...
    void testClassNamesResourceExists() {
        assertThat(getClass().getClassLoader().getResource(CLASS_NAMES_RESOURCE))
                .as("缺少 %s — 请确认 utils-support-models-onnx-table-detect jar 已在 classpath (test scope)",
                        CLASS_NAMES_RESOURCE)
                .isNotNull();
    }

    @Test
    @DisplayName("class.names.txt 恰好 1 行 ('table')")
    ...
    @Test
    @DisplayName("class.names.txt 内容 = 'table'")
    void testClassNameValue() throws IOException {
        ...
        List<String> lines = Arrays.stream(content.split("\\R"))
                ...
                .filter(s -> !s.isEmpty() && !s.startsWith("#"))
                .toList();
        assertThat(lines).containsExactly("table");
    }

    @Test
    @DisplayName("TableDetectionYolov8Translator 从 class.names.txt 加载类别")
    ...
    private static String readResource(String resource) throws IOException {
        ClassLoader cl = TableDetectModelPackageTest.class.getClassLoader();
