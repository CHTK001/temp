...
public class OnnxModelRegistrar implements ModelRegistrar {
 ...
 // 中文 BART-large：fnlp/bart-large-chinese，400M 中文书面语，int8 量化，downloadurl 模式（Z:\temp\onnx-zh-bart-int8）
        ...
        // 图像修复(MiGAN-perf)：动态 H/W uint8 双输入（image+mask），28MB 嵌入式，支持任意尺寸修复；适用图片修复、水印去除、物体移除
        ...
        // 图像修复(MiGAN-pipeline-v2)：动态 H/W uint8 双输入（image+mask），27MB 嵌入式，效果与 perf 版几乎一致（L1 误差 131.7 vs 131.8）；适用图片修复、水印去除、物体移除
