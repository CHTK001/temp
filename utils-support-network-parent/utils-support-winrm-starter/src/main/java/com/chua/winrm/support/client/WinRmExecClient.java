...
public class WinRmExecClient implements AutoCloseable {
    ...
    public WinRmExecClient connect() {
        try {
            ...
            throw new WinRMException("WinRM 连接失败[" + rootCause(e) + "]: " + username + "@" + host + ":" + port, e);
        }
        return this;
    }
     ...
     * 取异常链根因摘要
     * <p>
     * 上层（远程操作 Provider、Agent 部署服务）只把异常消息写进主机台账，堆栈会丢失，
     * 因此必须把真实失败原因放进消息本身。
     * </p>
     *
     * @param error 原始异常
     * @return 形如「SimpleClassName: message」的根因摘要
     */
    ...
    /**
     * 命令摘要，截断 -EncodedCommand 携带的超长 Base64 负载
     *
     * @param command 原始命令
     * @return 最长 160 字符的命令文本
     */
    private static String brief(String command) {
        if (command == null) {
            return "";
        }
        return command.length() <= 160 ? command : command.substring(0, 160) + "...";
    }
    ...
    public static class ExecOperation {
        ...
        public ExecResult execute() {
            try {
                ...
                throw new WinRMException("WinRM 命令执行失败[" + rootCause(e) + "]: " + brief(command), e);
            }
        }
    ...
    public static class TerminalOperation {
        ...
        public TerminalOperation send(String cmd) {
            ...
            try {
                ...
                throw new WinRMException("WinRM 终端执行失败: " + cmd, e);
            }
            return this;
        }
