...
/**
 * DataSync Agent 服务器，统一管理本地与远程 Agent 的注册、数据推送。
 * <p>
 * 本地 Agent 通过 {@link #register(DataSyncAgent)} 直接注册；
 * 远程 Agent 通过 SyncServer 的注册消息自动注册。
 * 数据推送时优先走本地调用，远程 Agent 通过 SyncServer 的 {@code sink:{agentId}} 主题下发。
 * </p>
 * <p>
 * 每个 Agent 注册后，其持有的 Source 和 Sink 会一并注册到 {@link DataSyncServer}，
...
public class DataSyncAgentServer implements AgentServerManager {
    ...
    /**
     * 创建 DataSyncAgentServer 实例
     * @param dataSyncServer dataSyncServer
     * @param SyncServer SyncServer
     * @param String String
     */
    ...
    private void unregisterAgentResources(DataSyncAgent agent) {
        // 从 DataSyncServer 注销 Agent 持有的 Source 和 Sink
     ...
     * <p>
     * 代理对象将远程 Source/Sink 的 ID 映射为本地代理对象，
     * 通过 SyncServer 与远程 Agent 进行网络通信。
     * </p>
     */
    ...
    /**
     * 远程 Source 代理，通过网络请求远程 Agent 读取数据。
     */
