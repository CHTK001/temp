...
public final class UebaEngineBuilder {
    ...
    /** 是否启用 MiniMind，null 表示默认启用 */
    private Boolean enableLlm;

    /** 模型目录（写入 ueba.model.dir 系统属性） */
    ...
    /**
     * 设置模型目录，写入系统属性 {@code ueba.model.dir}，AutoEncoder 与 LSTM 模型
     * 均从该目录加载。
     *
     * @param modelDir 模型目录，允许为 null（保持默认加载策略）
     * @return 当前构建器
     */
