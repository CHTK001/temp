...
/**
 * 盘云搜 (panyunso) 网盘搜索提供器
 *
 * @author CH
 * @since 4.0.0.42
 */
@Spi("panyunso")
public class PanyunsoResourceProvider extends AbstractResourceProvider {

    private static final String SEARCH_URL = "https://www.panyunso.com/?s=";

    public PanyunsoResourceProvider() { super(); }
    public PanyunsoResourceProvider(com.chua.common.support.datasearch.video.model.VideoSource vs) { super(vs); }
    ...
    public ReturnPageResult<VideoInfoResult> searchResource(VideoSearch videoSearch) {
        ...
        try {
            ...
            return ReturnPageResult.error("panyunso 搜索失败: " + e.getMessage());
        }
    }

    private void parseHtml(String html, List<VideoInfoResult> results) {
        java.util.regex.Pattern linkPat = java.util.regex.Pattern.compile("https?://[\\w.:/]+pan\\.[a-z\\.]+/[\\w/]+");
        java.util.regex.Matcher m = linkPat.matcher(html);
        int idx = 0;
        while (m.find() && idx++ < 10) {
            VideoInfoResult v = new VideoInfoResult();
            v.setVideoName("网盘搜索结果");
            v.setVideoPlatform("panyunso");
