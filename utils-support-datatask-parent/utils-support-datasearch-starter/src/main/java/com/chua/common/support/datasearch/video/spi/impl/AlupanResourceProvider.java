 ...
 */
@Spi("alupan")
public class AlupanResourceProvider extends AbstractResourceProvider {

    private static final String API_URL = "https://www.alipan.com/search";

    public AlupanResourceProvider() { super(); }
    public AlupanResourceProvider(com.chua.common.support.datasearch.video.model.VideoSource vs) { super(vs); }
    ...
    public ReturnPageResult<VideoInfoResult> searchResource(VideoSearch videoSearch) {
        ...
        try {
            ...
            return ReturnPageResult.error("alupan 搜索失败: " + e.getMessage());
        }
    }

    private void parseHtml(String html, List<VideoInfoResult> results) {
        java.util.regex.Pattern linkPat = java.util.regex.Pattern.compile("https?://[\\w.:/]+aliyundrive\\.com/[\\w/]+");
        java.util.regex.Matcher m = linkPat.matcher(html);
        int idx = 0;
        while (m.find() && idx++ < 10) {
            VideoInfoResult v = new VideoInfoResult();
            v.setVideoName("阿里云盘资源");
            v.setVideoPlatform("alupan");
