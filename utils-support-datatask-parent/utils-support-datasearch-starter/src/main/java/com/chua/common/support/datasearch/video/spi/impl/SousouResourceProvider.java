...
public class SousouResourceProvider extends AbstractResourceProvider {
    ...
    public ReturnPageResult<VideoInfoResult> searchResource(VideoSearch videoSearch) {
        ...
        try {
            ...
            String url = API_URL + "?kw=" + java.net.URLEncoder.encode(kw, "UTF-8");
            ...
            return ReturnPageResult.error("sousou 搜索失败: " + e.getMessage());
        }
    }

    private void parseHtml(String html, List<VideoInfoResult> results) {
        java.util.regex.Pattern linkPat = java.util.regex.Pattern.compile("https?://pan\\.(baidu|quark|aliyun)\\.com/[\\w/]+");
        java.util.regex.Matcher m = linkPat.matcher(html);
        int idx = 0;
        while (m.find() && idx++ < 10) {
            VideoInfoResult v = new VideoInfoResult();
            v.setVideoName("网盘资源");
            v.setVideoPlatform("sousou");
