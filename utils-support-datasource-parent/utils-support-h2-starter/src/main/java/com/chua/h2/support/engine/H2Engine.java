...
import com.chua.h2.support.flyway.H2Flyway;
...
public class H2Engine extends JdbcEngine {
    ...
    // ==================== Flylink 迁移（H2 DDL Normalizer） ====================

    /**
     * 获取 H2 迁移执行器。
     * <p>
     * 执行前自动将 MySQL/PostgreSQL 特有语法转换为 H2 兼容写法：
     * <ul>
     ...
     * @return H2Flyway 实例
     */
    public H2Flyway h2Flyway() {
        var dsObj = getDataSource();
        if (dsObj == null) throw new IllegalStateException("H2 引擎未配置数据源，请先调用 addDataSource()");
        return new H2Flyway((javax.sql.DataSource) dsObj.getSource());
    }
