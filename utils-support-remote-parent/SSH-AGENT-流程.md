...
| SSH 服务可用性 | 本机是否已有 sshd（22 端口可连） | 决定「转发」还是「自启」 |
...
agent 自采集平台类型 + SSH 服务可用性
         │
         ├── Linux ─────────────────────────────────────────┐
         │      ├─ 已有 sshd（22 可连）→ 转发（代理本机 sshd）  │
         │      └─ 无 sshd          → 自启 sshd（agent 拉起）  │
         │                                                    │
         ...
         └── Windows ────────────────────────────────────────┤
                ...
                → agent 直接启动 SSH 服务器（自启）            │
                                                              │
   三种结果统一：被控机具备可达的 SSH 服务（22）               │
   （转发=复用已有；自启=agent 补位）──────────────────────────┘
```

### 3.1 Linux
- **有 sshd（22 可连）**：agent **转发**——控制端的 SSH 会话经 agent 代理到本机已有 sshd，不重复起服务
- **无 sshd**：agent **自己启动 sshd**（拉起 SSH 服务进程，绑定 22），再走转发

### 3.2 Mac（其它类 Unix）
- **与 Linux 完全相同的逻辑**：有 sshd → 转发；无 → 自启
...
```
① 注册：agent 部署被控机 → 自采集（平台/sshd 可用性/凭据）→ 注册网关（上报 AgentInfo）
...
2. **新增**：`SshServiceProbe`——采集平台类型 + 检测本机 sshd（22 端口连通性）
