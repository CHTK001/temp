...
/**
 * 原始像素 RenderedImage 包装（网关转码用——不经 BufferedImage 中间态）。
 *
 * <p>包装原始 RGB 字节构造的 raster，供 ImageIO JPEG 编码器直接消费。</p>
 *
 * @author CH
 * @since 4.0.0.42
 */
final class GatewayRasterImage implements RenderedImage {
    ...
    GatewayRasterImage(Raster raster, int width, int height) {
        this.raster = raster;
        this.width = width;
        this.height = height;
