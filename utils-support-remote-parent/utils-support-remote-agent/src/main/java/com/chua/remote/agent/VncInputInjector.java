 ...
 * 键盘按键（keyPress/keyRelease）、鼠标移动与点击（mouseMove/mousePress/mouseRelease）。
...
public class VncInputInjector {
    ...
    public void inject(byte[] payload) {
        ...
        try {
            ...
            Map<?, ?> event = com.chua.common.support.lang.json.Json.fromJson(json, Map.class);
     ...
     */
    private void injectKeyEvent(Map<?, ?> event) {
     ...
     */
    private void injectMouseEvent(Map<?, ?> event) {
     ...
     */
    private void injectWheelEvent(Map<?, ?> event) {
        int x = toInt(event.get(FIELD_X));
        int y = toInt(event.get(FIELD_Y));
