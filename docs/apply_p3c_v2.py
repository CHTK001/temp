...
1. Add @Slf4j import and annotation to files using log but missing it
2. Add private constructor to pure static classes
3. Fix System.out.println -> log.info (preserving semicolon!)
...
def fix_file(path):
    ...
    # 1. Add @Slf4j if using log but missing it
    ...
    if has_log and (not has_slf4j or not has_import):
        if not has_import:
        ...
        if not has_slf4j:
            ...
            if class_match:
                ...
                content = content[:class_match.start(2)] + '@Slf4j\n' + content[class_match.start(2):]
                changed = True
        stats['slf4j'] += 1
