...
@Slf4j
@Spi("rust-h264")
public class RustH264VideoEncoder implements VideoEncoder {
    ...
    public synchronized void init(int width, int height, int fps) {
        ...
        this.started = this.encoderHandle != 0;
        log.info("[RustH264VideoEncoder] 已启动: {}x{} {}fps handle={}", width, height, fps, this.encoderHandle);
    }
    ...
    private void ensureInitialized(int w, int h, int f) {
        if (!started || this.encoderHandle == 0) {
            init(w, h, f);
        }
    }
    ...
    public synchronized byte[] encode(BufferedImage image) {
        ...
        try {
            ...
            byte[] encoded = NativeVideoCodec.h264Encode(this.encoderHandle, bgr, this.width, this.height);
            return encoded == null ? new byte[0] : encoded;
        } catch (Throwable e) {
            log.warn("[RustH264VideoEncoder] encode 失败: {}", e.getMessage());
            return new byte[0];
        }
    }
    ...
    public synchronized void close() {
        if (this.encoderHandle != 0) {
            NativeVideoCodec.h264EncoderFree(this.encoderHandle);
