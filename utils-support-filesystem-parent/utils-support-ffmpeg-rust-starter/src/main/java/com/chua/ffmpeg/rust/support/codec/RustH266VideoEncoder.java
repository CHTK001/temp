...
public class RustH266VideoEncoder implements VideoEncoder, EncodesFrame {
    ...
    * @param fps fps
    ...
    public synchronized void init(int width, int height, int fps) {
        ...
        this.encoderHandle = NativeVideoCodec.h266EncoderCreate(width, height, fps, DEFAULT_QUALITY, DEFAULT_PRESET, DEFAULT_PROFILE);
        this.started = encoderHandle != 0;
        log.info("[RustH266VideoEncoder] 已启动: {}x{} {}fps handle={}", width, height, fps, encoderHandle);
    }
    ...
    * @param f f
    ...
    public synchronized byte[] encode(BufferedImage image) {
        ...
        try {
            ...
            return NativeVideoCodec.h266Encode(encoderHandle, bgr, width, height);
        } catch (Throwable e) {
            log.warn("[RustH266VideoEncoder] encode(BufferedImage) 失败: {}", e.getMessage());
            return new byte[0];
        }
    }
    ...
    public synchronized byte[] encode(org.bytedeco.javacv.Frame frame) {
        ...
        try {
            ...
            log.warn("[RustH266VideoEncoder] encode(Frame) 失败: {}", e.getMessage());
            return new byte[0];
        }
    }
