...
/**
 * 基于原生 Rust JNI 的 H.265/HEVC 编码器。
 *
 * @author CH
 * @since 4.0.0.42
 */
@Slf4j
@Spi("rust-h265")
public class RustH265VideoEncoder implements VideoEncoder, EncodesFrame {
    ...
    public synchronized void init(int width, int height, int fps) {
        ...
        this.encoderHandle = NativeVideoCodec.h265EncoderCreate(width, height, fps, DEFAULT_QUALITY, DEFAULT_PRESET, DEFAULT_PROFILE);
        this.started = encoderHandle != 0;
        log.info("[RustH265VideoEncoder] 已启动: {}x{} {}fps handle={}", width, height, fps, encoderHandle);
    }
    ...
    public synchronized byte[] encode(BufferedImage image) {
        ...
        try {
            ...
            return NativeVideoCodec.h265Encode(encoderHandle, bgr, width, height);
        } catch (Throwable e) {
            log.warn("[RustH265VideoEncoder] encode(BufferedImage) 失败: {}", e.getMessage());
            return new byte[0];
        }
    }
    ...
    public synchronized byte[] encode(org.bytedeco.javacv.Frame frame) {
        ...
        try {
            ...
            log.warn("[RustH265VideoEncoder] encode(Frame) 失败: {}", e.getMessage());
            return new byte[0];
        }
    }
    ...
    public synchronized void close() {
        if (encoderHandle != 0) {
            NativeVideoCodec.h265EncoderFree(encoderHandle);
            encoderHandle = 0;
        }
        started = false;
    }
