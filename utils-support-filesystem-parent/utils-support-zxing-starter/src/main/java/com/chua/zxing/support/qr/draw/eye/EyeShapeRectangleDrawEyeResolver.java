...
public class EyeShapeRectangleDrawEyeResolver extends AbstractDrawEyeResolver {
    ...
    public void draw(int x, int y, QrCodeRenderHelper.DetectLocation detectLocation) {
        ...
        if (detectLocation == QrCodeRenderHelper.DetectLocation.LT) {
            path.quadTo(x1, y1, x1 + arcw1, y1);
            path.lineTo(x1 + w1, y1);
            path.lineTo(x1 + w1, y1 + h1 - arch1);
            path.quadTo(x1 + w1, y1 + h1, x1 + w1 - arcw1, y1 + h1);
            path.lineTo(x1 + arcw1, y1 + h1);
            path.quadTo(x1, y1 + h1, x1, y1 + h1 - arch1);

            path2.quadTo(x2, y2, x2 + arcw2, y2);
            path2.lineTo(x2 + w2, y2);
            path2.lineTo(x2 + w2, y2 + h2 - arch2);
            path2.quadTo(x2 + w2, y2 + h2, x2 + w2 - arcw2, y2 + h2);
            path2.lineTo(x2 + arcw2, y2 + h2);
            path2.quadTo(x2, y2 + h2, x2, y2 + h2 - arch2);
            //右上角
        } else if (detectLocation == QrCodeRenderHelper.DetectLocation.RT) {
            path.lineTo(x1, y1);
