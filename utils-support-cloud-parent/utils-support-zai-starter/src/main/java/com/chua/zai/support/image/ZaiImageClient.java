package com.chua.zai.support.image;
...
@Slf4j
@Spi({"zai"})
public class ZaiImageClient implements ImageClient {

    /**
    * ZAI 平台默认 API 地址
     */
    private static final String DEFAULT_URL = "https://api.zai.com";
    ...
    * @param setting 客户端配置
     */
    public ZaiImageClient(ImageClientSetting setting) {
...
public BufferedImage generate(String prompt) {
        ...
        try {
            ...
            String url = normalizeBaseUrl() + "/v1/images/generations";
            ...
            if (response.statusCode() != 200) {
                throw new RuntimeException("ZAI 图片生成请求失败: " + response.statusCode() + " - " + response.body());
            }
            String body = response.body();
            return parseAndDownloadImage(body);
        } catch (IOException e) {
            throw new RuntimeException("ZAI 图片生成请求失败", e);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new RuntimeException("ZAI 图片生成请求被中断", e);
        }
    }
