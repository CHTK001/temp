...
- Collapse enhancements:
  ...
  - Tests added: `CollapseHttpClientTest` (`executeAsync` failure broadcast via Mono onError, `closeDelegate` three-state behavior), `CollapseFlowTest` (executorFactory callback actually used), `CollapseAutoConfigurationDiscoveryTest` (AutoConfiguration.imports registration + real SpringApplication discovery & binding)
