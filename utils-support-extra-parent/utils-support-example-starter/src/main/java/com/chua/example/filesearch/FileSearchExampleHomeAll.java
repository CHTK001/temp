 ...
 * @since 2026-07-27
 */
public class FileSearchExampleHomeAll {
    ...
    public static void main(String[] args) {
        ...
        if (!rootFile.exists() || !rootFile.isDirectory()) {
            System.err.println("[FileSearchExampleHomeAll] root not exists or not a directory: " + root);
            System.exit(1);
        }

        FileSearchService service = FileSearchService.getInstance();
        if (!service.isAvailable()) {
            System.err.println("[FileSearchExampleHomeAll] service not available");
            System.exit(2);
        }
    ...
    private static void printStep(String label, String value) {
        System.out.println("[FileSearchExampleHomeAll] " + label + "=" + value);
    }
}
