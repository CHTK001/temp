...
import com.chua.deeplearning.support.onnx.seg.EfficientSamSegmentTranslator;
...
/**
 * EfficientSAM 本地分割示例 — 提示框分割（Image + bbox → 前景掩码）。
 *
 * <p>模型来自 {@code utils-support-models-onnx-efficientsam} jar（约 40MB，FP32），
 * 完全离线推理，无需联网。适合边缘设备 / 嵌入式部署。</p>
 *
 * <h2>用法</h2>
 * <pre>
 *   # 自检模式：生成合成测试图（白底红方块）并校验掩码覆盖率
 *   java EfficientSamExample --test
 *
 *   # 指定图片 + 提示框（像素坐标，可省略 bbox 则用整图）
 *   java EfficientSamExample --file "C:/photo.jpg" --box "100,100,300,300"
 *
 *   # 输出掩码
 *   java EfficientSamExample --file "C:/photo.jpg" --box "100,100,300,300" --out "C:/mask.png"
 * </pre>
 *
 * @author CH
 * @since 4.0.0.42
 */
@Slf4j
public class EfficientSamExample {
    ...
    private static final String DEFAULT_TEST_IMAGE =
            System.getProperty("java.io.tmpdir") + "efficientsam_test.png";

    /**
     * 默认掩码输出路径
     */
    private static final String DEFAULT_MASK_IMAGE =
            System.getProperty("java.io.tmpdir") + "efficientsam_mask.png";

    public static void main(String[] args) {
        CommandLine cli = CommandLine.parse(args)
                .program("EfficientSamExample")
                .register("file", "f", "输入图片路径")
                .register("box", "b", "提示框 x1,y1,x2,y2（像素坐标）")
                .register("out", "o", "掩码输出路径（默认: 临时目录 efficientsam_mask.png）")
        ...
        EfficientSamExample example = new EfficientSamExample();
        boolean passed;
    ...
    public boolean runSelfTest() {
        try {
            ...
            log.info("[{}] EfficientSAM 自检 → 耗时 {}ms, 框内掩码覆盖率 {:.0%}",
                    passed ? "PASS" : "FAIL", elapsed, coverage);
            log.info("      测试图: {}, 掩码图: {}", DEFAULT_TEST_IMAGE, DEFAULT_MASK_IMAGE);
            return passed;
        } catch (Exception e) {
            log.error("[FAIL] EfficientSAM 自检异常: {}", e.getMessage(), e);
            return false;
        }
    }
