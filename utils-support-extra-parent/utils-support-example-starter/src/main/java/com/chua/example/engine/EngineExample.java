...
/**
 * 统一 EngineExample：通过 -Dengine.type=xxx 切换引擎，使用统一 User 实体和统一 CRUD 测试流。
 *
 * @author CH
 * @since 2024/12/12
 */
...
public class EngineExample {
    ...
    private static void assertNotContains(String label, List<?> list, Object expected) {
        boolean contains = list.stream().anyMatch(item -> {
            try {
                Method m = item.getClass().getMethod("name");
                return Objects.equals(m.invoke(item), expected);
        ...
        printResult(label + " (not contains " + expected + ")", !contains ? "PASS" : "FAIL");
        if (contains) {
            throw new AssertionError(label + ": list still contains " + expected);
        }
    }
    ...
    private static Engine setupDataSource(String engineType) {
        return switch (engineType) {
            case "neo4j" -> setupNeo4j();
            case "sqlite" -> setupSqlite();
            case "mysql"  -> setupMySql();
            case "lucene" -> setupLucene();
            case "file"   -> setupFile();
            case "memory" -> setupInMemory();
            case "duckdb" -> setupDuckDB();
            case "hbase"  -> setupHBase();
            case "influxdb" -> setupInfluxDb();
            case "parquet" -> setupParquet();
    ...
    private static Engine setupNeo4j() {
        ...
        String password = System.getProperty("neo4j.password", "neo4j123");
    ...
    private static Engine setupSqlite() {
        String dbFile = System.getProperty("sqlite.file", "data/example.db");
    ...
    private static Engine setupMySql() {
        ...
        String database = System.getProperty("mysql.database", "test_db");
        String username = System.getProperty("mysql.user", "root");
        String password = System.getProperty("mysql.password", "123456");
    ...
    private static Engine setupLucene() {
        String indexPath = System.getProperty("lucene.index", "data/lucene-index");
    ...
    private static Engine setupTablesaw() {
        String csvFile = System.getProperty("tablesaw.file", "data/test.csv");
    ...
    public static void main(String[] args) throws Exception {
        String engineType = System.getProperty("engine.type", "sqlite");
