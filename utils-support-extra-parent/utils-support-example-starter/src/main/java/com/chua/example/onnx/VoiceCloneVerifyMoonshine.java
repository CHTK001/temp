 ...
 */
public final class VoiceCloneVerifyMoonshine {

    private VoiceCloneVerifyMoonshine() {}

    public static void main(String[] args) throws Exception {
        String text = args.length > 0 ? args[0] : "Hello world this is a test";
        System.out.println("===== TTS→STT 验证管线（moonshine-base） =====");
        System.out.println("[pipeline] 文本: " + text);

        // Step 1: TTS
        System.out.println("[tts] 合成中...");
        byte[] audio = synthesizeTts(text);
        System.out.println("[tts] 字节数: " + audio.length);
        ...
        try {
            ...
            System.out.println("[stt] 转写: [" + transcript + "]");
            System.out.println("[verify] 长度: TTS=" + text.length() + " STT=" + (transcript != null ? transcript.length() : 0));
        } catch (Exception e) {
    ...
    static Path extractMoonshineModel() throws Exception {
        ...
        if (extracted == 0) throw new RuntimeException("No moonshine resources found");
        return modelDir;
    }
