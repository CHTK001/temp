...
public interface Environment {
     ...
     * 查找顺序遵循优先级规则（手动设置属性 &gt; ConfigSource 属性 &gt; 默认值）。
     * 如果所有来源均未找到该 key，则返回 null。</p>
     *
     * @param key 配置键，如 "server.port"，不可为 null
     ...
     * 自动转换为目标类型。支持 String、Integer、Boolean、Long 等常见类型。</p>
     *
     * @param key        配置键，不可为 null
     * @param targetType 目标类型，不可为 null
     * @param <T>        泛型类型
     ...
     * @param <T>          泛型类型
     * @return 转换后的配置值，不存在时返回 defaultValue
     */
     ...
     *
     * @param listener 配置变更监听器，不可为 null
     */
     ...
     *
     * @param propertySource 配置源，不可为 null
     */
