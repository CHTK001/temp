...
public interface LoadBalance extends AutoCloseable {
     ...
     */
    static @Nullable LoadBalance auto(String type, BalanceConfig config) {
     ...
     */
    <T> @Nullable T select(@Nullable List<T> values);
     ...
     * @return 当前实例（链式调用）
     */
    LoadBalance addNode(@Nullable Node node);
     ...
     * @return 当前实例（链式调用）
     */
    default LoadBalance addNode(@Nullable Collection<?> node) {
        if (null == node) {
            return this;
        }
        for (Object node1 : node) {
