...
/**
 * TCP 实现 ScatterNodeServer。
 *
 * <p>包装 {@link com.chua.common.support.network.sync.impl.TcpSyncServer}，
 * 通过 SPI 创建 "tcp" 类型的 SyncServer 实例，同一端口承载数据同步与心跳。</p>
 *
 * @author CH
 * @since 4.0.0.42
 */
@Slf4j
@Spi("tcp")
public class TcpScatterNodeServer implements ScatterNodeServer {
    ...
    /**
     * 构建 TCP Scatter 节点服务器。
     *
     * @param setting 节点配置
     */
    public TcpScatterNodeServer(ScatterSetting setting) {
     ...
     * @param port 端口号
     */
    public TcpScatterNodeServer(String host, int port) {
        this.setting = ServerSetting.defaults();
        this.setting.setHost(host);
        this.setting.setPort(port);
        this.setting.setProtocol(PROTOCOL_TCP);
        this.syncServer = ServiceProvider.of(SyncServer.class).getNewExtension(PROTOCOL_TCP, setting);
    }
    ...
    public void start() throws Exception {
        if (syncServer != null) {
            syncServer.start();
            log.info("TCP Scatter 节点服务器已启动: {}:{}", setting.getHost(), setting.getPort());
        }
    }
